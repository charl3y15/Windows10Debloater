...
Function Start-Debloat {
    ...
    Get-AppxPackage -AllUsers | Where-Object { $_.Name -NotMatch $WhitelistedApps } | Remove-AppxPackage -ErrorAction SilentlyContinue  | Out-File $LogPath -Append

    $AppxRemoval = Get-AppxProvisionedPackage -Online | Where-Object { $_.PackageName -NotMatch $WhitelistedApps }
    ForEach ( $App in $AppxRemoval) {

        Remove-AppxProvisionedPackage -Online -PackageName $App.PackageName | Out-File $LogPath -Append
...
Function Remove-Keys {
    ...
    ForEach ($Key in $Keys) {
        Write-Output "Removing $Key from registry"  | Out-File $LogPath -Append
        Remove-Item $Key -Recurse -ErrorAction SilentlyContinue | Out-File $LogPath -Append
    }
}
...
Function Protect-Privacy {
    ...
    Write-Output "Disabling Windows Feedback Experience program" | Out-File $LogPath -Append
    $Advertising = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AdvertisingInfo'
    If (Test-Path $Advertising) {
        Set-ItemProperty $Advertising -Name Enabled -Value 0 -Verbose | Out-File $LogPath -Append
    }

    #Stops Cortana from being used as part of your Windows Search Function
    Write-Output "Stopping Cortana from being used as part of your Windows Search Function" | Out-File $LogPath -Append
    $Search = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'
    If (Test-Path $Search) {
        Set-ItemProperty $Search -Name AllowCortana -Value 0 -Verbose | Out-File $LogPath -Append
    }
